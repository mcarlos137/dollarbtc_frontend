...
class Transactions extends Component {
  ...
  makeTableData = (movements) => {
    ...
    Object.entries(movements).forEach(([key, value]) => {
      tx = {};
      if (
        value.addedAmount !== undefined &&
        value.addedAmount.currency === "BTC"
      ) {
        ...
        Object.entries(value).forEach(([k, v]) => {
          ...
          if (k === "addedAmount") {
            ...
            if (v === "WITHDRAW") {
            ...
            } else if (v === "CURRENCY_CHANGE") {
              tx.description = this.state.translator(
                "wallet.tx.operationType.currencyChange"
              );
            } else if (v === "FAST_CHANGE") {
              tx.description = this.state.translator(
                "wallet.tx.operationType.fastChange"
              );
            } else if (v === "DEPOSIT") {
              tx.description = this.state.translator(
              ...
              tx.description = this.state.translator(
                "wallet.tx.operationType.transferFromBALANCEToMC"
              );
            } else if (v === "TRANSFER_FROM_MCBALANCE_TO_BALANCE") {
              tx.description = this.state.translator(
                "wallet.tx.operationType.transferFromMCToBALANCE"
              );
            } else if (v === "RECEIVE") {
              tx.description = this.state.translator(
        ...
        value.substractedAmount !== undefined &&
        value.substractedAmount.currency === "BTC"
      ) {
        tx.date = new Date(key.split("__")[0]).getTime();
        Object.entries(value).forEach(([k, v]) => {
          tx.OTCid = "";
    ...
    this.setState({
      ...
      allIncomingBTC: this.formatBTC(allIncoming),
      allOutcomingBTC: this.formatBTC(allOutcoming),
      renderNow: true,
    });
  };
  formatDate(date) {
  ...
  render() {
    ...
    const transactionTableHeaders = [
      ...
      {
        Header: t("wallet.tx.table.headers.amountBTC"),
        accessor: "amountBTC",
