...
class AddConfiguration extends Component {
	...
	getPaymentMethodsByCurrency = (currency) => {
		...
		otc
			...
			.then((resp) => {
				...
				console.log(paymentMethodsArrayUniqueKey);
				this.setState({
			...
			.then((res) => {
				...
				Object.entries(res.data).forEach(([currency, sourceReferences]) => {
					if (currency === symbol) {
						...
						if (currencyCurrent !== undefined && symbol !== "ETH" && symbol !== "USDT") {
	...
	addConfig = () => {
      ...
      if (this.state.paymentMethodTypeToAdd.length > 0) {
